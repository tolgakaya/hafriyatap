    ...
    public class TekMusteri
    {
        ...
        private List<ServisHesapRepo> kararlar()
        {
            return (from s in dc.servicehesaps
                    ...
                    where s.MusteriID == custID && s.iptal == false && m.hesapid == s.HesapID
                    ...
                    select new ServisHesapRepo
                    {
                        ...
                        cihaz = (s.cihaz_id == null && s.makine_id == null) ? "Servis" : (s.makine_id == null ? (s.adet.ToString() + "-" + s.cihaz.cihaz_adi) : (s.cihaz_adi + "-" + s.calisma_saati.ToString() + "-" + s.tarife_kodu)),
