        ...
        private List<GGR> satis_raporu_genel()
        {
            ...
            IEnumerable<CariDetayx> hesaplar = from s in dc.servicehesaps
                                               ...
                                               select new CariDetayx
                                               {
                                                   ...
                                                   islem_adres = s.makine_id == null ? (s.adet.ToString() + "Adet") : (s.calisma_saati + "-" + s.tarife_kodu),
                                                   cesit = "karar",
                                                   masraf_tipi = (s.cihaz_id == null && s.makine_id == null) ? "Servis" : (s.makine_id == null ? "Malzeme" : s.cihaz_adi)
        ...
        private List<GGR> satis_raporu()
        {
            ...
            IEnumerable<rapor_kalemi> hesaplar = from s in dc.servicehesaps
                                                 ...
                                                 select new rapor_kalemi
                                               {
                                                   ...
                                                   aciklama = s.Aciklama,
                                                   musteriAdi = s.customer.Ad,
                                                   tutarr = (decimal)s.Yekun,
                                                   tarih = s.TarihZaman,
                                                   islem = s.IslemParca,
                                                   islem_turu = s.cihaz_adi,
                                                   islem_adres = s.makine_id == null ? (s.adet.ToString() + " " + s.birim) : ("Ondalık: " + s.calisma_saati.ToString() + " " + s.tarife_kodu + " Net: " + s.sure_aciklama),
                                                   cesit = "karar",
                                                   masraf_tipi = (s.cihaz_id == null && s.makine_id == null) ? "Servis" : (s.makine_id == null ? "Malzeme" : s.cihaz_adi)
            ...
            IEnumerable<rapor_kalemi> satis = from s in dc.satislars
                                              ...
                                              select new rapor_kalemi
                                            {
                                                ...
                                                islem = "Peşin Satış",
                                                islem_turu = s.cihaz.cihaz_adi,
                                                islem_adres = s.adet.ToString() + s.birim,
                                                cesit = "pesin",
                                                masraf_tipi = s.cihaz_id == null ? "Servis" : "Ürün/Parça"
