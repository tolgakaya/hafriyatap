...
namespace ServisDAL
{
    public class Hareket
    {
        ...
        public List<CariDetayYeni> CariDetayRYeni(int musteriID, int gun)
        {
            // cari detay için
            ...
            IEnumerable<CariDetayYeni> hesaplar = from s in db.servicehesaps
                                                  ...
                                                  select new CariDetayYeni
                                                  {
                                                      ...
                                                      borc = s.tamirci_id == musteriID ? 0 : s.Yekun,
                                                      alacak = s.tamirci_id == musteriID ? s.toplam_maliyet : 0,
                                                      tarih = (DateTime)s.Onay_tarih,
                                                      islem = s.IslemParca,
                                                      konu = (s.cihaz_id == null && s.makine_id == null) ? "Servis" : (s.makine_id == null ? (s.adet.ToString() + " " + s.birim + "-" + s.cihaz.cihaz_adi) : (s.cihaz_adi + "-" + s.calisma_saati.ToString() + "-" + s.tarife_kodu)),

                                                      kullanici = s.updated == null ? s.inserted : (s.inserted + "-" + s.updated)
                                                  };
            IEnumerable<CariDetayYeni> alimlar = from s in db.alims
                                                 where s.iptal == false && s.CustID == musteriID && s.alim_tarih >= tarih
                                                 select new CariDetayYeni
                                                 {
                                                     MusteriID = s.CustID,
                                                     aciklama = s.aciklama,
            ...
            return odeme_tahsilat.Union(internet_fatura).Union(hesaplar).Union(alimlar).OrderByDescending(x => x.tarih).ToList();
        }
        public List<CariDetayYeni> CariDetayREski(int musteriID, int gun)
        {
            ...
            IEnumerable<CariDetayYeni> hesaplar = from s in db.servicehesaps
                                                  ...
                                                  select new CariDetayYeni
                                                  {
                                                      ...
                                                      tarih = (DateTime)s.Onay_tarih,
                                                      islem = s.IslemParca,
                                                      konu = (s.cihaz_id == null && s.makine_id == null) ? "Servis" : (s.makine_id == null ? (s.adet.ToString() + "-" + s.cihaz.cihaz_adi) : (s.cihaz_adi + "-" + s.calisma_saati.ToString() + "-" + s.tarife_kodu)),
