        ...
        protected void CreateUser_Click(object sender, EventArgs e)
        {
            ...
            if (result.Succeeded)
            {
                using (Radius.radiusEntities dc = Radius.MyContext.Context(txtConfig.Text))
                {
                    ...
                    ay.kullanicisinir = Int32.Parse(kullanicisinir.Text);
                    ay.makinesinir = Int32.Parse(makinesinir.Text);
                    if (lisanspaket.SelectedValue.Equals("light"))
                    {
                        ay.calismatipi = true;
                    }
                    else
                    {
                        ay.calismatipi = false;
                    }
                    ay.adres = Adres.Text;
                    ay.email = Email.Text;
                    ay.fifo = true;
